...
const ShowShopInfo = (info) => {
    ...
    return(
                    ...
                    <div className="i">
                        ...
                        {s?<Link onClick={()=>{window.scrollTo({top:0})}} to={'/basket'} className="p-r-b">В корзину</Link>:<button onClick={()=>{info.create({key:id,item:1,from:info.item.title});setS('yes');}} type="button" className="p-r-b">Выбрать</button>}
                        </div>
                    </div>
                    </div>
    );
